...
class DoubleDribbleDetector:
    def __init__(self):
        ...
        # Initialize the video capture object to capture video from file or camera.
        # Change this line to use a video file instead of webcam:
        self.cap = cv2.VideoCapture("driblle.mp4")  # Use video file
        # self.cap = cv2.VideoCapture(0)  # Use webcam (default)
    ...
    def process_frame(self, frame):
        ...
        # Get the detected keypoints with proper error handling
        try:
            ...
            # Check if we have at least one person detected with sufficient keypoints
            if keypoints_np.shape[0] == 0 or keypoints_np.shape[1] <= max(self.body_index.values()):
                print("No human detected.")
                return pose_annotated_frame, False

            # Extract the coordinates of the wrists (x, y coordinates only)
            left_wrist = keypoints_np[0][self.body_index["left_wrist"]][:2]  # Only x,y coordinates
            right_wrist = keypoints_np[0][self.body_index["right_wrist"]][:2]  # Only x,y coordinates
